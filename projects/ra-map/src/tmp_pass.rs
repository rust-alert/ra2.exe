...
use crate::pass_grid::PassGrid;
use crate::theater::{theater_ini_name, theater_tmp_extension};
use crate::tileset::parse_tileset_ini;
use crate::{ground_passable, MapInfo};

/// 读取剧院 TMP 的 `terrain_type`，对水/岩/墙等不可走格调用 `PassGrid::seal_land_types`。
///
/// 返回新封格数量。
pub fn seal_pass_grid_from_tmp(
    ...
    let Ok(ini_bytes) = source.read(theater_ini_name(map.theater)) else {
        return 0;
    };
