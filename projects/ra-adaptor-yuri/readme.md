...
1. 理解 edition profile 的职责边界（「它是什么」）。
2. 看清与 `ra-adaptor`、引擎启动链的关系（「在仓库中的位置」）。
3. 在编排层引用 `profile()` 与 `looks_like`（「如何使用」）。
4. 资源表字段与探测逻辑（「内部设计」）。
5. 构建测试与许可证。
...
本 crate 采用 **MPL-2.0**。资源文件名来自尤里的复仇公开安装布局的常见约定；使用本表驱动装载时，请确保你拥有合法游戏内容副本。
