...
fn mcv_deploy_open_seeds_slice_funds() {
    ...
    let id = case.session.expect_game().world.entity_id_at(0).expect("seeded mcv");
    let (type_id, _) = case.session.expect_game().world.ecs_identity(id).expect("mcv identity");
...
fn mcv_deploy_through_session_becomes_yard() {
    ...
    assert_eq!(case.session.expect_game().world.entity_id_at(0), Some(id));
    let (type_id, kind) = case.session.expect_game().world.ecs_identity(id).expect("deployed yard");
