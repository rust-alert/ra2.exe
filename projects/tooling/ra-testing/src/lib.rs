...
//! 与产品相同的 `Session`、`BattleState` / `BattleSession` 和 `GameCommand` 路径推进。
...
pub use gui::{
    GuiAction, GuiAutomationPlan, GuiExpectation, GuiPoint, pre_alpha_acceptance_capture_names, pre_alpha_acceptance_capture_plan,
    standard_duel_gui_plan,
};
