...
use ra_adaptor::{
    ResourceChain, build_runtime_definitions, load_rules_chain, load_rules_chain_with_overlays, rules_system_from_layered_ini_bytes,
};
...
[SA]\nVerses=100%,100%,100%,100%,100%,100%,100%,100%,100%,100%,100%\n";
    let top = b"[MTNK]\nOwner=Alliance\nCost=800\n";
    let rules = rules_system_from_layered_ini_bytes(GameEdition::Ra2, &[base.as_slice(), top.as_slice()], &[]).expect("layered rules");
...
impl AssetSource for MapAssetSource {
    fn read(&self, relative: &str) -> RaResult<Vec<u8>> {
        self.files.get(relative).map(|b| b.to_vec()).ok_or_else(|| RaError::MissingFile(relative.to_string()))
    }
}
...
[SA]\nVerses=100%,100%,100%,100%,100%,100%,100%,100%,100%,100%,100%\n";
    ...
    let source = MapAssetSource {
        files: HashMap::from([(chain.rules_ini, base.as_slice()), (chain.art_ini, b"".as_slice()), ("MPBattle.ini", overlay.as_slice())]),
    };
...
fn load_rules_chain_with_overlays_missing_file_errors() {
    let base = b"[VehicleTypes]\n0=MTNK\n[MTNK]\nCost=700\n";
    let chain = ResourceChain::for_edition(GameEdition::Ra2);
    let source = MapAssetSource { files: HashMap::from([(chain.rules_ini, base.as_slice()), (chain.art_ini, b"".as_slice())]) };
...
fn load_rules_chain_with_art_overlays_overrides_foundation() {
    ...
    let chain = ResourceChain::for_edition(GameEdition::Ra2);
    let source = MapAssetSource {
        files: HashMap::from([(chain.rules_ini, rules.as_slice()), (chain.art_ini, art_base.as_slice()), ("artmod.ini", art_top.as_slice())]),
    };
...
[SA]\nVerses=100%,100%,100%,100%,100%,100%,100%,100%,100%,100%,100%\n";
    ...
    let source = MapAssetSource {
        files: HashMap::from([("rules.ini", underlay.as_slice()), (chain.rules_ini, primary.as_slice()), (chain.art_ini, b"".as_slice())]),
    };
...
[90mm]\nDamage=50\nROF=8\nRange=6\nWarhead=SA\n\
[SA]\nVerses=100%,100%,100%,100%,100%,100%,100%,100%,100%,100%,100%\n";
    let source_primary_only =
        MapAssetSource { files: HashMap::from([(chain.rules_ini, primary_only.as_slice()), (chain.art_ini, b"".as_slice())]) };
