...
use ra_layout::{
    CAMPAIGN_BUTTON_IDS, CHOOSE_MAP_BUTTON_IDS, EXIT_CONFIRM_BUTTON_IDS, LOAD_SCREEN_BUTTON_IDS, LayoutId, LayoutSnapshot,
    MAIN_MENU_BUTTON_IDS, NETWORK_BUTTON_IDS, OPTIONS_BUTTON_IDS, Rect, RectPx, SINGLE_PLAYER_BUTTON_IDS, SKIRMISH_LOBBY_BUTTON_IDS,
    solve_battle_hud, solve_battle_pause, solve_campaign, solve_choose_map, solve_exit_confirm, solve_load_screen, solve_network_page,
    solve_options_page, solve_shell_page, solve_skirmish_lobby,
};
...
impl RenderPlan {
    ...
    pub fn solid_placeholders_from_snapshot(snapshot: &LayoutSnapshot, root_id: &str) -> Self {
        let commands = snapshot
            ...
            .map(|e| RenderCommand::SolidRect { id: e.id.clone(), rect: e.layout.rect, color: [80, 80, 80, 255] })
            .collect();
        Self { commands }
    }
    ...
    pub fn rect_of(&self, id: &str) -> Option<Rect> {
        self.commands.iter().find(|cmd| cmd.id().0 == id).map(RenderCommand::rect)
    }
    ...
    pub fn button_sprite_plan(&self, button_ids: &[&str]) -> Self {
        self.retaining_ids(button_ids).promote_ids_to_sprites(button_ids)
    }

    /// 丢掉指定 id 的命令（例如 HUD 战术区底边 `command_bar`）。
    pub fn excluding_ids(&self, skip: &[&str]) -> Self {
        Self { commands: self.commands.iter().filter(|cmd| !skip.iter().any(|s| cmd.id().0.as_str() == *s)).cloned().collect() }
    }

    /// 只保留指定 id 的命令。
    pub fn retaining_ids(&self, keep: &[&str]) -> Self {
        Self { commands: self.commands.iter().filter(|cmd| keep.iter().any(|s| cmd.id().0.as_str() == *s)).cloned().collect() }
    }
    ...
    pub fn recolor_ids(&self, ids: &[&str], color: [u8; 4]) -> Self {
        Self {
            commands: self
                ...
                .iter()
                .map(|cmd| match cmd {
                    RenderCommand::SolidRect { id, rect, color: prev } => {
                        let color = if ids.iter().any(|s| id.0.as_str() == *s) { color } else { *prev };
                        RenderCommand::SolidRect { id: id.clone(), rect: *rect, color }
                    }
    ...
    pub fn promote_ids_to_sprites(&self, ids: &[&str]) -> Self {
        Self {
            commands: self
                ...
                .iter()
                .map(|cmd| match cmd {
                    RenderCommand::SolidRect { id, rect, .. } if ids.iter().any(|s| id.0.as_str() == *s) => {
                        RenderCommand::SpriteRect { id: id.clone(), rect: *rect, slot: id.0.clone() }
                    }
                    other => other.clone(),
                })
                .collect(),
    ...
    pub fn shell_page_placeholders(root_id: &str, stacked_ids: &[&str], bottom_id: Option<&str>) -> Self {
    ...
    pub fn for_original_screen(screen: OriginalScreen) -> Option<Self> {
        Some(match screen {
            OriginalScreen::MainMenu => Self::shell_page_placeholders("main_menu", &MAIN_MENU_BUTTON_IDS[..5], Some(MAIN_MENU_BUTTON_IDS[5])),
            OriginalScreen::SinglePlayerMenu => {
                Self::shell_page_placeholders("single_player", &SINGLE_PLAYER_BUTTON_IDS[..3], Some(SINGLE_PLAYER_BUTTON_IDS[3]))
            }
    ...
    pub fn diagnostic_for_original_screen(screen: OriginalScreen) -> Option<Self> {
        ...
        Some(Self::for_original_screen(screen)?.excluding_ids(&["background", "movie"]).recolor_ids(rail, [196, 148, 48, 255]))
    }
}
