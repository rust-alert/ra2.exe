...
fn main_menu_hit_single_player() {
    ...
    let action = hit_action(OriginalScreen::MainMenu, &[], 0, None, (924.0, 282.0), 1024.0, 768.0, false);
...
fn main_menu_hit_options_and_exit_cells() {
    ...
    assert_eq!(hit_action(OriginalScreen::MainMenu, &[], 0, None, (ox, oy), 1024.0, 768.0, false), Some(MenuAction::OpenOptions));
    assert_eq!(hit_action(OriginalScreen::MainMenu, &[], 0, None, (ex, ey), 1024.0, 768.0, false), Some(MenuAction::Exit));
}
...
fn exit_confirm_ok_and_cancel_cells() {
    ...
    assert_eq!(hit_action(OriginalScreen::ExitConfirm, &[], 0, None, (ox, oy), 1024.0, 768.0, false), Some(MenuAction::ConfirmExit));
    assert_eq!(hit_action(OriginalScreen::ExitConfirm, &[], 0, None, (cx, cy), 1024.0, 768.0, false), Some(MenuAction::Back));
}

#[test]
fn load_screen_hides_hits_while_loading() {
    let loading = hits_for(OriginalScreen::LoadScreen, &[], 0, false);
    assert!(loading.is_empty());
    let failed = hits_for(OriginalScreen::LoadScreen, &[], 0, true);
...
fn disabled_network_not_hit() {
    ...
    let action = hit_action(OriginalScreen::MainMenu, &[], 0, None, (924.0, 335.0), 1024.0, 768.0, false);
...
fn lobby_start_button_is_hit() {
    ...
    let action = hit_action(OriginalScreen::SkirmishLobby, &maps, 0, Some("mp03t4.map"), (sx as f64, sy as f64), 1024.0, 768.0, false);
    assert_eq!(action, Some(MenuAction::StartSkirmish));
}

#[test]
fn hover_index_tracks_enabled_button() {
    assert_eq!(hover_index(OriginalScreen::MainMenu, &[], 0, None, (924.0, 282.0), 1024.0, 768.0, false,), Some(0));
    // 禁用钮仍可悬停（底栏 STT 提示），但不可点击。
    assert_eq!(hover_index(OriginalScreen::MainMenu, &[], 0, None, (924.0, 335.0), 1024.0, 768.0, false,), Some(1));
}
...
fn physical_cursor_with_logical_window_misses_on_hidpi() {
    ...
    let logical = hit_action(OriginalScreen::MainMenu, &[], 0, None, (924.0, 282.0), 1024.0, 768.0, false);
    let physical_mixed = hit_action(OriginalScreen::MainMenu, &[], 0, None, (1386.0, 423.0), 1024.0, 768.0, false);
...
fn campaign_side_and_difficulty_are_hit() {
    ...
    assert_eq!(hit_action(OriginalScreen::Campaign, &[], 0, None, (ax, ay), 1024.0, 768.0, false), Some(MenuAction::SelectCampaignAllied));
    assert_eq!(hit_action(OriginalScreen::Campaign, &[], 0, None, (tx, ty), 1024.0, 768.0, false), Some(MenuAction::CycleCampaignDifficulty));
    assert_eq!(hit_action(OriginalScreen::Campaign, &[], 0, None, (bx, by), 1024.0, 768.0, false), Some(MenuAction::Back));
...
fn choose_map_use_and_list_row_are_hit() {
    ...
    assert_eq!(hit_action(OriginalScreen::ChooseMap, &maps, 0, Some("mp03t4.map"), (ux, uy), 1024.0, 768.0, false), Some(MenuAction::UseMap));
    let (mx, my) = to_win(layout.map_list.x + 8, layout.map_list.y + 8);
    assert_eq!(
        hit_action(OriginalScreen::ChooseMap, &maps, 0, Some("mp03t4.map"), (mx, my), 1024.0, 768.0, false),
