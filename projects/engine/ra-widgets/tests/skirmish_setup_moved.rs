...
fn checkbox_toggle_and_track_drag() {
    ...
    assert_eq!(
        s.on_press(r.x + 2, r.y + 2, 1),
        Some(SkirmishLobbyHit::Toggle(SkirmishCheckbox::ShortGame))
    );
    assert!(!s.short_game);
    let t = snap_rect(&snap, "track_speed");
    assert_eq!(
        s.on_press(t.x + 4, t.y + 4, 1),
        Some(SkirmishLobbyHit::Track(SkirmishTrackbar::GameSpeed))
    );
