...
use ra_layout::{CHOOSE_MAP_LIST_ROW_H, choose_map_list_row_rect, rect_px_from_snapshot, solve_choose_map};
...
fn list_row_center(list_id: &str, row: usize) -> (f64, f64) {
    ...
    ((row_rect.x + row_rect.w / 2) as f64, (row_rect.y + CHOOSE_MAP_LIST_ROW_H / 2) as f64)
}
...
fn choose_map_hits_use_snapshot_button_rects() {
    ...
    let action = hit_action(OriginalScreen::ChooseMap, &maps, 0, None, (722.0, 262.0), 800.0, 600.0, 0, false);
    assert_eq!(action, Some(MenuAction::UseMap));

    let cancel = hit_action(OriginalScreen::ChooseMap, &maps, 0, None, (722.0, 556.0), 800.0, 600.0, 0, false);
...
fn choose_map_mode_and_map_rows_are_hit() {
    ...
    assert_eq!(
        hit_action(OriginalScreen::ChooseMap, &maps, 2, None, (mode_x, mode_y), 800.0, 600.0, 0, false),
        Some(MenuAction::SelectMode(0))
    );
    assert_eq!(hit_action(OriginalScreen::ChooseMap, &maps, 2, None, (map0_x, map0_y), 800.0, 600.0, 0, false), Some(MenuAction::SelectMap(0)));
    assert_eq!(hit_action(OriginalScreen::ChooseMap, &maps, 2, None, (map1_x, map1_y), 800.0, 600.0, 0, false), Some(MenuAction::SelectMap(1)));
}
...
fn choose_map_scroll_shifts_hit_rows() {
    ...
    assert_eq!(hit_action(OriginalScreen::ChooseMap, &maps, 1, None, (map_x, map_y), 800.0, 600.0, 0, false), Some(MenuAction::SelectMap(0)));
    assert_eq!(hit_action(OriginalScreen::ChooseMap, &maps, 1, None, (map_x, map_y), 800.0, 600.0, 3, false), Some(MenuAction::SelectMap(3)));
}
