...
use ra_layout::{SKIRMISH_LOBBY_BUTTON_IDS, rect_px_from_snapshot, solve_skirmish_lobby};
use ra_map::{BootMapCandidate, Theater};
use ra_widgets::{input::hit::*, menu_action::MenuAction, original_screen::OriginalScreen};
...
fn exit_confirm_ok_and_cancel_cells() {
    use ra_layout::{EXIT_CONFIRM_BUTTON_IDS, rect_px_from_snapshot, solve_exit_confirm};
...
fn campaign_side_and_difficulty_are_hit() {
    ...
    assert_eq!(
        hit_action(OriginalScreen::Campaign, &[], 0, None, (tx, ty), 1024.0, 768.0, 0, false),
...
fn choose_map_use_and_list_row_are_hit() {
    ...
    assert_eq!(
        hit_action(OriginalScreen::ChooseMap, &maps, 0, Some("mp03t4.map"), (ux, uy), 1024.0, 768.0, 0, false),
