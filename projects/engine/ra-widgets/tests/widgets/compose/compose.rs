...
use ra_layout::{
    MAIN_MENU_BUTTON_IDS, OPTIONS_BUTTON_IDS, RightPanelChrome, SINGLE_PLAYER_BUTTON_IDS, SKIRMISH_LOBBY_BUTTON_IDS, rect_px_from_snapshot,
    solve_shell_page, solve_skirmish_lobby,
};
...
fn compose_hides_button_caption_while_wave_frames_active() {
    ...
    let frames = [10u16, 10, 10, 10, 10, 10];
    let waving = compose_main_menu_page(
        &decoded,
        800,
        600,
        None,
...
fn compose_campaign_uses_back_id() {
    use ra_layout::{CAMPAIGN_BUTTON_IDS, rect_px_from_snapshot, solve_campaign};
...
fn compose_empty_tiles_use_wave_sdbtnanm_instead_of_static_bkgd() {
    ...
    let empty_ti = (0..RightPanelChrome::shell_defaults().tile_count())
        .find(|&ti| {
            ...
            !MAIN_MENU_BUTTON_IDS.iter().any(|id| {
                let b = rect_px_from_snapshot(&snap, id);
                b.w > 0 && b.y == y
            })
        })
