...
fn command_button_tip_keys_cover_retail_slots() {
    ...
    assert_eq!(command_button_csf_tooltip(3), Some("TIP:DEPLOY"));
    assert_eq!(command_button_csf_tooltip(4), Some("TIP:ATTACKMOVE"));
    assert_eq!(command_button_csf_tooltip(5), Some("TIP:GUARD"));
    assert_eq!(command_button_csf_tooltip(6), Some("TIP:STOP"));
    assert_eq!(command_button_csf_tooltip(7), Some("TIP:PLANNINGMODE"));
    assert_eq!(command_button_csf_tooltip(8), None);
    assert_eq!(command_bar_shp_index("TypeSelect"), Some(3));
    assert_eq!(command_bar_shp_index("PlanningMode"), Some(9));
    assert_eq!(command_bar_shp_index("Cheer"), Some(10));
}
...
fn country_lobby_display_resolves_uiname_csf() {
    let csf = csf_with_entries(&[("Name:Africans", "利比亚"), ("Name:Alliance", "韩国"), ("NAME:AMERICANS", "美国")]);
