...
fn set_lobby_sides_preserves_selected_names() {
    ...
    req.set_lobby_sides(vec!["Americans".into(), "Russians".into(), "French".into(), "Korea".into()]);
    assert_eq!(req.side, "Russians");
...
fn load_screen_uses_explicit_names_only() {
    assert_eq!(load_screen_background_shp_resolved(800, Some("ls800haihead.shp")).as_deref(), Some("ls800haihead.shp"));
    assert_eq!(load_screen_background_shp_resolved(640, Some("ls800haihead.shp")).as_deref(), Some("ls640haihead.shp"));
    assert_eq!(load_screen_background_shp_resolved(800, None), None);
    assert_eq!(load_screen_palette_resolved(Some("mplshh.pal"), |_| true).as_deref(), Some("mplshh.pal"));
    assert_eq!(load_screen_palette_resolved(None, |n| n == "mpls.pal").as_deref(), Some("mpls.pal"));
    assert_eq!(load_screen_brief_csf_key("Guild1", Some("STT:PlayerSideHaihead")), "STT:PlayerSideHaihead");
...
fn ui_faction_chrome_is_open_by_mix_index_only() {
    ...
    assert_eq!(epsilon.radar_shp_pal_candidates()[1], ("radary.shp", "radaryuri.pal"));

    // 任意多阵营：index 5 / 6 直接生成 sidecNN。
    let fifth = UiFactionChrome::from_mix_index(5, false);
    assert_eq!(fifth.sidebar_mix(), "sidec05.mix");
    assert_eq!(fifth.sidebar_mix_candidates(), vec!["sidec05md.mix".to_string(), "sidec05.mix".to_string()]);
...
fn eva_fallback_stems_follow_tag_not_allied_first() {
    ...
    assert_eq!(eva_fallback_sample_names("EVA_BattleControlTerminated", Some("Yuri")), vec!["cyur015".to_string(), "CYUR015".to_string()]);
    assert_eq!(eva_fallback_sample_names("EVA_MissionAccomplished", Some("Russian")), vec!["csof013".to_string(), "CSOF013".to_string()]);
