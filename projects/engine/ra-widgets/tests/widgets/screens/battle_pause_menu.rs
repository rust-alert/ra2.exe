...
use ra_layout::{BATTLE_PAUSE_MENU_BUTTON_IDS, BattleHudChromeMetrics};
use ra_widgets::{
...
fn battle_pause_menu_hits_resume_and_options_from_snapshot() {
    ...
    assert_eq!(hit_at(800, 600, metrics, resume.x + 4, resume.y + 4), Some(BattlePauseMenuHit::Resume));
    assert_eq!(hit_at(800, 600, metrics, options.x + 4, options.y + 4), Some(BattlePauseMenuHit::Options));
}

#[test]
fn compose_battle_pause_menu_dims_left_and_paints_rail() {
    let metrics = BattleHudChromeMetrics::sidec01();
    let page = compose_battle_pause_menu_overlay(800, 600, None, Some("options"), None, None, None, metrics).unwrap();
