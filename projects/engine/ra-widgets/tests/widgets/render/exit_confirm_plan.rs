...
use ra_layout::{EXIT_CONFIRM_BUTTON_IDS, Point2, solve_exit_confirm};
...
fn exit_confirm_render_plan_rects_match_snapshot_hits() {
    ...
    assert_eq!(snap.get(ok_id).map(|e| e.layout.rect), plan.rect_of(ok_id));
    assert_eq!(snap.get("dialog").map(|e| e.layout.rect), plan.rect_of("dialog"));

    let cancel = plan.rect_of(cancel_id).expect("cancel");
    let hit = snap.hit_test(Point2 { x: cancel.x + 4.0, y: cancel.y + 4.0 }).expect("hit cancel");
