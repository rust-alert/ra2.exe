...
use ra_layout::{Point2, solve_skirmish_lobby};
...
fn skirmish_lobby_render_plan_rects_match_snapshot_hits() {
    ...
    assert_eq!(snap.get("map_preview").map(|e| e.layout.rect), plan.rect_of("map_preview"));

    let hit = snap.hit_test(Point2 { x: start.x + 4.0, y: start.y + 4.0 }).expect("hit start");
