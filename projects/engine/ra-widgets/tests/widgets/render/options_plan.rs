...
fn options_page_render_plan_rects_match_snapshot_hits() {
    ...
    assert_eq!(snap.get("accept").map(|e| e.layout.rect), plan.rect_of("accept"));
    assert_eq!(snap.get("track_detail").map(|e| e.layout.rect), plan.rect_of("track_detail"));
    assert_eq!(snap.get("content").map(|e| e.layout.rect), plan.rect_of("content"));

    let accept = plan.rect_of("accept").expect("accept");
    let hit = snap.hit_test(Point2 { x: accept.x + 4.0, y: accept.y + 4.0 }).expect("hit accept");
