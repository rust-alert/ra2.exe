...
use ra_layout::{NETWORK_BUTTON_IDS, Point2, solve_network_page};
...
fn network_render_plan_rects_match_snapshot_hits() {
    ...
    for id in NETWORK_BUTTON_IDS {
        assert_eq!(snap.get(id).map(|e| e.layout.rect), plan.rect_of(id), "{id}");
    }

    let hit = snap.hit_test(Point2 { x: 200.0, y: 350.0 }).expect("hit back");
