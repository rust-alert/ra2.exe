...
use ra_layout::{
    COMMAND_BAR_BUTTON_IDS, COMMAND_BAR_H, COMMAND_BUTTON_W, COMMAND_LENDCAP_W, Point2, battle_hud_world_viewport, rect_px_from_snapshot,
    solve_battle_hud,
};
...
fn battle_hud_render_plan_rects_match_snapshot_hits() {
    ...
    assert_eq!(snap.get("repair").map(|e| e.layout.rect), plan.rect_of("repair"));
    ...
    let cmd_hit = snap.hit_test(Point2 { x: cmd0.x as f32 + 4.0, y: cmd0.y as f32 + 4.0 }).expect("hit cmd0");
