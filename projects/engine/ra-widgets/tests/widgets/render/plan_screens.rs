...
fn for_original_screen_covers_menu_pages() {
    ...
    for screen in screens {
        ...
        assert!(!plan.commands.is_empty(), "{} should yield placeholder commands", screen.as_str());
...
fn diagnostic_recolors_rail_buttons() {
    let plan = RenderPlan::diagnostic_for_original_screen(OriginalScreen::MainMenu).expect("main menu");
    let exit = plan
        .commands
        .iter()
        .find_map(|cmd| match cmd {
            ra_widgets::RenderCommand::SolidRect { id, color, .. } if id.0 == "exit" => Some(*color),
            _ => None,
        })
        .expect("exit command");
...
fn recolor_ids_only_touches_named_commands() {
    ...
    let use_map = tinted
        ...
        .iter()
        .find_map(|cmd| match cmd {
            ra_widgets::RenderCommand::SolidRect { id, color, .. } if id.0 == "use_map" => Some(*color),
            _ => None,
        })
        .expect("use_map");
    let cancel = tinted
        ...
        .iter()
        .find_map(|cmd| match cmd {
            ra_widgets::RenderCommand::SolidRect { id, color, .. } if id.0 == "cancel" => Some(*color),
            _ => None,
        })
        .expect("cancel");
...
fn promote_ids_to_sprites_keeps_rect_and_skips_raster() {
    ...
    let plan = full.retaining_ids(&["use_map"]).promote_ids_to_sprites(&["use_map"]);
    assert!(matches!(
...
fn button_sprite_plan_and_paint_sprites_into() {
    ...
    let sprite = RgbaImage::from_raw(2, 2, vec![255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255]).expect("sprite");
    let mut page = RgbaImage::from_raw(800, 600, vec![0u8; 800 * 600 * 4]).expect("page");
    plan.paint_sprites_into(&mut page, |slot| if slot == "use_map" { Some(&sprite) } else { None });
