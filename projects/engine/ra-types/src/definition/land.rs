//! 规范陆地类型（INI `Land=` / TMP `terrain_type`）。
...
impl LandType {
    ...
    /// 由 rules `Land=` 字符串解析（大小写不敏感）；空 / 未知返回 `None`。
...
impl<'de> Deserialize<'de> for LandType {
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for LandVisitor {
            type Value = LandType;

            fn expecting(&self, f: &mut fmt::Formatter) -> fmt::Result {
                f.write_str("Land= type name")
            }

            fn visit_str<E: de::Error>(self, v: &str) -> Result<Self::Value, E> {
                Ok(LandType::parse_name(v).unwrap_or(LandType::Clear))
            }
        ...
        deserializer.deserialize_any(LandVisitor)
    }
}
