//! 暂停菜单专属 layout：一棵树含 world + pause hub 槽 + SIDEBTTN 列表。
...
#[test]
fn battle_pause_owns_hub_slots_not_cameo_grid() {
    ...
    // 对局 cameo / 页签 / 修理出售不进 pause 树。
    assert!(snap.get("cameo_band").is_none());
    assert!(snap.get("repair").is_none());
    assert!(snap.get("tab00").is_none());
...
#[test]
fn battle_pause_world_hub_and_packed_list_at_800x600() {
    ...
    assert_eq!(resume.y + resume.h, list_band.y + list_band.h);
}
