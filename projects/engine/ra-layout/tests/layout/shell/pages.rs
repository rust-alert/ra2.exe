...
fn skirmish_lobby_matches_game_exe_dialog_0x102() {
    ...
    assert_eq!(rect_px_from_snapshot(&snap, "start"), RectPx::new(644, 241, 156, 42));
    assert_eq!(rect_px_from_snapshot(&snap, "choose_map"), RectPx::new(644, 283, 156, 42));
...
fn choose_map_matches_game_exe_dialog_0x6b() {
    ...
    assert_eq!(rect_px_from_snapshot(&snap, "use_map"), RectPx::new(644, 241, 156, 42));
    assert_eq!(rect_px_from_snapshot(&snap, "create_random"), RectPx::new(644, 283, 156, 42));
    assert_eq!(rect_px_from_snapshot(&snap, "cancel"), RectPx::new(644, 535, 156, 42));
    assert_eq!(rect_px_from_snapshot(&snap, "map_preview"), RectPx::new(644, 37, 144, 112));
    assert_eq!(rect_px_from_snapshot(&snap, "title"), RectPx::new(635, 2, 162, 16));
    assert_eq!(rect_px_from_snapshot(&snap, "map_name_plate"), RectPx::new(644, 157, 156, 84));
    assert_eq!(rect_px_from_snapshot(&snap, "game_type"), RectPx::new(649, 167, 135, 16));
    assert_eq!(rect_px_from_snapshot(&snap, "map_label"), RectPx::new(649, 189, 135, 33));
...
fn campaign_matches_fsbkgdlg_side_origins() {
    use ra_layout::{CAMPAIGN_BUTTON_IDS, CAMPAIGN_SIDE_IDS, rect_px_from_snapshot, solve_campaign};
...
fn exit_confirm_centers_pudlgbgn_panel() {
    ...
    assert_eq!(rect_px_from_snapshot(&snap, "dialog"), RectPx::new(175, 137, EXIT_CONFIRM_DIALOG_W, EXIT_CONFIRM_DIALOG_H));
    // 正文左上锚点区；按钮原点取 DLU，尺寸取 `mnbttn` 126×25。
    assert_eq!(rect_px_from_snapshot(&snap, "prompt"), RectPx::new(235, 202, 330, 81));
    assert_eq!(rect_px_from_snapshot(&snap, EXIT_CONFIRM_BUTTON_IDS[0]), RectPx::new(486, 356, EXIT_CONFIRM_BUTTON_W, EXIT_CONFIRM_BUTTON_H));
    assert_eq!(rect_px_from_snapshot(&snap, EXIT_CONFIRM_BUTTON_IDS[1]), RectPx::new(486, 421, EXIT_CONFIRM_BUTTON_W, EXIT_CONFIRM_BUTTON_H));
}
...
fn shell_page_layouts_ignore_viewport_size() {
    ...
    for (w, h) in [(640u32, 480u32), (800, 600), (1280, 720), (2560, 1440)] {
        let _ = (w, h);
        let main = ra_layout::solve_shell_page("main_menu", &MAIN_MENU_BUTTON_IDS[..5], Some(MAIN_MENU_BUTTON_IDS[5]));
        assert_eq!(rect_px_from_snapshot(&main, "exit"), RectPx::new(644, 535, 156, 42), "{w}x{h} exit");

        let lobby = ra_layout::solve_skirmish_lobby();
        assert_eq!(rect_px_from_snapshot(&lobby, "start"), RectPx::new(644, 241, 156, 42), "{w}x{h} start");
        ...
        assert_eq!(rect_px_from_snapshot(&campaign, "allied"), RectPx::new(30, 26, 570, 135), "{w}x{h} allied");
