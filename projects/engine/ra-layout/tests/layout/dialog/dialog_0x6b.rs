...
use ra_layout::{Point2, Rect, solve_choose_map};

#[test]
fn dialog_0x6b_snapshot_matches_choose_map_golden_rects() {
    let snap = solve_choose_map();

    assert_eq!(snap.get("use_map").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 241.0, 156.0, 42.0)));
    assert_eq!(snap.get("create_random").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 283.0, 156.0, 42.0)));
    assert_eq!(snap.get("cancel").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 535.0, 156.0, 42.0)));
    assert_eq!(snap.get("map_preview").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 37.0, 144.0, 112.0)));
    assert_eq!(snap.get("title").map(|e| e.layout.rect), Some(Rect::from_xywh(635.0, 2.0, 162.0, 16.0)));
    assert_eq!(snap.get("game_type").map(|e| e.layout.rect), Some(Rect::from_xywh(649.0, 167.0, 135.0, 16.0)));
    assert_eq!(snap.get("map_label").map(|e| e.layout.rect), Some(Rect::from_xywh(649.0, 189.0, 135.0, 33.0)));
    ...
    assert!((eng_mid - mid).abs() <= 2.0, "engagement mid {eng_mid} should match lists mid {mid}");
    assert!((engagement.width - (map_list.x + map_list.width - game_type_list.x)).abs() <= 1.0);

    assert_eq!(snap.get("lower_strip").map(|e| e.layout.rect), Some(Rect::from_xywh(0.0, 568.0, 632.0, 32.0)));
    assert_eq!(snap.get("status_help").map(|e| e.layout.rect), Some(Rect::from_xywh(15.0, 579.0, 455.0, 20.0)));

    assert_eq!(snap.hit_test(Point2 { x: 650.0, y: 260.0 }).map(|e| e.id.0.as_str()), Some("use_map"));
    assert_eq!(snap.hit_test(Point2 { x: 650.0, y: 540.0 }).map(|e| e.id.0.as_str()), Some("cancel"));
}
