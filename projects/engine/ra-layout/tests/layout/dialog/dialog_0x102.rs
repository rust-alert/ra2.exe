...
use ra_layout::{Point2, Rect, solve_skirmish_lobby};

#[test]
fn dialog_0x102_snapshot_matches_skirmish_golden_rects() {
    let snap = solve_skirmish_lobby();

    assert_eq!(snap.get("start").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 241.0, 156.0, 42.0)));
    assert_eq!(snap.get("choose_map").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 283.0, 156.0, 42.0)));
    assert_eq!(snap.get("back").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 535.0, 156.0, 42.0)));
    ...
    assert_eq!(snap.get("map_name_plate").map(|e| e.layout.rect), Some(Rect::from_xywh(644.0, 157.0, 156.0, 84.0)));
    ...
    let team = snap.get("team_face_0").map(|e| e.layout.rect);
    assert!(team.is_none(), "RA2 lobby has no team column");
    ...
    assert_eq!(snap.hit_test(Point2 { x: 650.0, y: 250.0 }).map(|e| e.id.0.as_str()), Some("start"));
}
