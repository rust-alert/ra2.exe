...
fn order_attack_and_detects_victor() {
    ...
    let foe = session.expect_battle().nearest_hostile(EntityId(1)).unwrap();
    assert_eq!(foe, EntityId(2));
    session.expect_battle_mut().order_attack(&[EntityId(1)], foe);
    for _ in 0..80 {
        session.tick(&engine.runtime());
        if session.expect_battle().sole_victor().is_some() {
            break;
        }
    }
...
fn image_to_cell_uses_preview_origin() {
    ...
    let ix = cx - (-100.0);
    let iy = cy - (-50.0);
    assert_eq!(session.expect_battle().image_to_cell(ix, iy), Some((5, 4)));
}
...
fn snapshot_includes_screen_coords_and_selection() {
    ...
    let z = session.expect_battle().world.pass_grid.cell_height(5, 4);
...
fn pump_advances_fixed_hz_ticks() {
    ...
    assert_eq!(session.expect_battle().world.tick, 1);
...
fn remote_digest_mismatch_pauses() {
    ...
    bad.hash ^= 0xff;
    assert!(!session.expect_battle_mut().apply_remote_digest(&bad));
    assert!(session.expect_battle().paused);
    assert!(session.expect_battle().pause_reason.is_some());
    assert_eq!(session.pump(&engine.runtime(), 1.0), 0);
    session.expect_battle_mut().resume();
    assert!(!session.expect_battle().paused);
...
fn remote_digest_tick_mismatch_is_not_success() {
    ...
    assert!(!session.expect_battle().paused, "tick 不一致不应当成哈希冲突暂停");
}
