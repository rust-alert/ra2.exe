...
#[test]
fn prepared_map_seed_binds_ai_trigger_condition_object_id() {
    let defs = defs_with_mtnk();
    let mtnk = defs.techno.get("MTNK").expect("MTNK").id;
    let mut map = map_with_size();
    map.scripting.task_forces.push(MapTaskForce {
    ...
    map.scripting.ai_triggers.push(MapAiTrigger {
        ...
        team: "TM1".into(),
        owner_house: "Americans".into(),
        condition: ra_types::AiTriggerConditionKind::OwnOwns,
        condition_object: "MTNK".into(),
        ..Default::default()
    });
    let world = battle_from_defs(GameEdition::Ra2, defs, map);
    assert_eq!(world.prepared.ai_triggers.len(), 1);
    assert_eq!(world.prepared.ai_triggers[0].condition_object_id, Some(mtnk));
}

#[test]
fn unbound_ai_trigger_condition_object_rejects_battle_seed() {
    let defs = defs_with_mtnk();
    let mut map = map_with_size();
    map.scripting.task_forces.push(MapTaskForce {
        id: "TF1".into(),
...
#[test]
fn prepared_map_seed_binds_destroy_trigger_action_id() {
    let defs = defs_with_mtnk();
    let mut map = map_with_size();
    map.scripting.triggers.push(MapTrigger {
        id: "TR1".into(),
    ...
    map.scripting.actions.push(MapAction {
        id: "TR1".into(),
        commands: vec![MapActionCommand {
            kind: MapActionKind::DestroyTrigger,
            params: ["0".into(), "TR2".into(), String::new(), String::new(), String::new(), String::new(), String::new()],
        }],
    });
    let world = battle_from_defs(GameEdition::Ra2, defs, map);
...
#[test]
fn unbound_destroy_trigger_action_rejects_battle_seed() {
    let defs = defs_with_mtnk();
    let mut map = map_with_size();
    map.scripting.triggers.push(MapTrigger {
        id: "TR1".into(),
...
#[test]
fn prepared_map_seed_binds_flash_team_action_id() {
    let defs = defs_with_mtnk();
    let (mut map, team) = seed_map_with_team("Americans");
    map.scripting.actions.push(MapAction {
        id: "TR1".into(),
        commands: vec![MapActionCommand {
            kind: MapActionKind::FlashTeam,
            params: ["0".into(), team, String::new(), String::new(), String::new(), String::new(), String::new()],
        }],
    });
    let world = battle_from_defs(GameEdition::Ra2, defs, map);
...
#[test]
fn unbound_flash_team_action_rejects_battle_seed() {
    let defs = defs_with_mtnk();
    let mut map = map_with_size();
    map.scripting.triggers.push(MapTrigger {
        id: "TR1".into(),
