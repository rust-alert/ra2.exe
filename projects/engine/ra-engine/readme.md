...
本包集成测试只有一个根：`tests/engine/main.rs`（Cargo 二进制名 `engine`）。其下按与 `src/` 相同的七轴用目录 + `mod.rs` 组织（
`runtime` / `state` / `spatial` / `gameplay` / `lifecycle` / `presentation` / `persistence`），共用 `common`。不使用
`#[path]`，也不使用「同名 `axis.rs` + `axis/`」双轨。轴上预留模块不等于该能力测例已齐。
