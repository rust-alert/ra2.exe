...
///
/// 按稳定 [`TypeId`] 判定，避免运行时再按类型名回查。
pub fn is_type_eligible_id(defs: &RuntimeDefinitions, player: TechTreePlayer<'_>, living: &HashSet<TypeId>, type_id: TypeId) -> bool {
    let Some(techno) = defs.techno.get_by_id(type_id)
    else {
        return false;
    };
