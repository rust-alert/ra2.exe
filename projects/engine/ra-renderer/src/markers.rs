...
#[doc(hidden)]
pub const MAX_SPRITE_VERTICES: u64 = 8192 * 6;
#[doc(hidden)]
pub const MAX_LINE_VERTICES: u64 = 8192 * 6;
...
impl MarkerGpu {
    ...
    pub fn create(
        ...
        assets: &DecodedSelectionOverlay,
    ) -> Option<Self> {
        ...
        for (i, frame) in frames.iter().enumerate() {
            ...
            slots.push(AtlasSlot { u0, v0, u1, v1, w: fw as f32, h: fh as f32 });
        }
    ...
    pub fn write_from_world(&mut self, queue: &wgpu::Queue, world: &RenderWorld, camera: &Camera, surface_w: u32, surface_h: u32) {
        ...
        let yellow = if world.condition_yellow > 0.0 { world.condition_yellow } else { 0.5 };
        let red = if world.condition_red > 0.0 { world.condition_red } else { 0.25 };
        ...
        for u in world.units.values().filter(|u| !u.dead) {
            ...
            let ratio = if u.max_health == 0 { 0.0 } else { (u.health as f32 / u.max_health as f32).clamp(0.0, 1.0) };
...
#[doc(hidden)]
pub fn blit_at(dst: &mut RgbaImage, src: &RgbaImage, ox: u32, oy: u32) {
...
#[doc(hidden)]
pub fn push_sprite_quad(out: &mut Vec<SpriteVertex>, camera: &Camera, sw: f32, sh: f32, cx: f32, cy: f32, slot: &AtlasSlot) {
    ...
    out.extend_from_slice(&[
        SpriteVertex { pos: p00, uv: [slot.u0, slot.v0] },
        SpriteVertex { pos: p10, uv: [slot.u1, slot.v0] },
        SpriteVertex { pos: p11, uv: [slot.u1, slot.v1] },
        SpriteVertex { pos: p00, uv: [slot.u0, slot.v0] },
        SpriteVertex { pos: p11, uv: [slot.u1, slot.v1] },
        SpriteVertex { pos: p01, uv: [slot.u0, slot.v1] },
    ]);
}

#[doc(hidden)]
pub fn push_unit_pipbrd(
    ...
    let slot = if infantry { slots.get(1) } else { slots.first() };
    let Some(slot) = slot
    else {
        return;
    };
    let delta = u.bracket_delta as f32;
    let (ox, oy) = if infantry { (11.0, delta - 25.0) } else { (1.0, delta - 26.0) };
    push_sprite_quad(out, camera, sw, sh, cx + ox, cy + oy, slot);
}

#[doc(hidden)]
pub fn push_unit_pips(
    out: &mut Vec<SpriteVertex>,
    camera: &Camera,
    sw: f32,
    sh: f32,
    ...
    let max_pips = if infantry { UNIT_PIPS_INFANTRY } else { UNIT_PIPS_VEHICLE };
    ...
    let delta = u.bracket_delta as f32;
    let (start_x, start_y) = if infantry { (-5.0, delta - 24.0) } else { (-15.0, delta - 25.0) };
    for i in 0..filled {
...
pub fn push_building_pips(
    ...
    for i in 0..num {
        ...
        let slot = if i < filled { filled_slot } else { empty };
...
#[doc(hidden)]
pub fn push_building_brackets(out: &mut Vec<LineVertex>, camera: &Camera, sw: f32, sh: f32, sx: f32, sy: f32, u: &RenderUnit) {
    let fw = u.foundation_w.max(1) as f32;
    let fh = u.foundation_h.max(1) as f32;
...
#[doc(hidden)]
pub fn building_box_corners(sx: f32, sy: f32, fw: f32, fh: f32, z_screen: f32) -> ([Pt; 4], [Pt; 4]) {
    let cx = sx + (fw - fh) * 15.0;
    let cy = sy + (fw + fh) * 7.5 - 15.0;
    let ground = [
        Pt { x: cx - (fw + fh) * 15.0, y: cy + (fh - fw) * 7.5 },
        Pt { x: cx + (fw - fh) * 15.0, y: cy + (fw + fh) * 7.5 },
        Pt { x: cx + (fh - fw) * 15.0, y: cy - (fw + fh) * 7.5 },
        Pt { x: cx + (fw + fh) * 15.0, y: cy - (fh - fw) * 7.5 },
    ];
    let roof = [
        Pt { x: ground[0].x, y: ground[0].y - z_screen },
        Pt { x: ground[1].x, y: ground[1].y - z_screen },
        Pt { x: ground[2].x, y: ground[2].y - z_screen },
        Pt { x: ground[3].x, y: ground[3].y - z_screen },
    ];
    (ground, roof)
}

#[doc(hidden)]
pub fn quarter(a: Pt, b: Pt) -> Pt {
    Pt { x: ((a.x * 3.0 + b.x) * 0.25).trunc(), y: ((a.y * 3.0 + b.y) * 0.25).trunc() }
}

#[doc(hidden)]
pub fn push_stub_line(out: &mut Vec<LineVertex>, camera: &Camera, sw: f32, sh: f32, a: Pt, b: Pt, color: [f32; 4]) {
    let q = quarter(a, b);
    push_pixel_line(out, camera, sw, sh, a, q, color);
}

#[doc(hidden)]
pub fn push_pixel_line(out: &mut Vec<LineVertex>, camera: &Camera, sw: f32, sh: f32, a: Pt, b: Pt, color: [f32; 4]) {
...
#[doc(hidden)]
pub fn draw_bresenham(out: &mut Vec<LineVertex>, camera: &Camera, sw: f32, sh: f32, x0: i32, y0: i32, x1: i32, y1: i32, color: [f32; 4]) {
    let mut x = x0;
    let mut y = y0;
    let dx = (x1 - x0).abs();
    let dy = -(y1 - y0).abs();
...
#[doc(hidden)]
pub fn push_pixel_rect(out: &mut Vec<LineVertex>, camera: &Camera, sw: f32, sh: f32, x: f32, y: f32, color: [f32; 4]) {
