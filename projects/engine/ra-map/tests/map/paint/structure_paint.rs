...
use ra_map::{
    ArtRules, MapEntity, MapEntityKind, MapInfo, StructureAnimMode, StructurePaintHintTable, TerrainImage,
...
fn empty_structures_noop() {
    ...
    assert_eq!(
        paint_map_structures(&EmptySource, &map, &mut image, &ArtRules::default(), &mut StructurePaintHintTable::default(), &|p, _| p.clone(), StructureAnimMode::BodyOnly),
        (0, 0)
    );
}
...
";
    ...
    let painted = paint_map_structures(
        ...
        &map,
        &mut image,
        &ArtRules::load(&source, "art.ini", "rules.ini"), &mut StructurePaintHintTable::default(), &|p, _| p.clone(),
...
";
    ...
    let source = MapSource { files };
    let bank = collect_structure_anim_bank(&source, &map, &ArtRules::load(&source, "art.ini", "rules.ini"), &mut StructurePaintHintTable::default(), &|p, _| p.clone());
...
";
    ...
    let clip = load_structure_buildup_clip(&source, &map, &art_rules, &mut StructurePaintHintTable::default(), "GACNST", "Americans", 3, 4, &|p, _| p.clone()).expect("buildup clip");
...
";
    ...
    assert!(bank.layers.iter().any(|l| l.frames.len() == 2 && l.rate_ms == 300), "ActiveAnim flag layer missing");
}
...
fn missing_structure_body_paints_magenta_marker() {
    ...
    let mut image = TerrainImage::blank(256, 256);
    let (shp, mark) = paint_map_structures(&source, &map, &mut image, &ArtRules::load(&source, "art.ini", "rules.ini"), &mut StructurePaintHintTable::default(), &|p, _| p.clone(), StructureAnimMode::BodyOnly);
