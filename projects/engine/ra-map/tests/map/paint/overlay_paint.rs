...
fn empty_overlays_noop() {
    ...
    assert_eq!(
        paint_map_overlays(&EmptySource, &map, &mut image, &PaintDefinitions::default(), &|_| None, &|_| false, &|_| None, OverlayLayerFilter::All, ),
        (0, 0)
    );
}
...
fn theater_overlay_uses_theater_palette() {
    ...
    let (shp, mark) = paint_map_overlays(
        ...
        &map,
        &mut image,
        &PaintDefinitions::load(&source, "art.ini", "rules.ini"),
...
fn tiberium_overlay_keeps_temperat_colors_without_hsv_remap() {
    ...
    let mut plain = TerrainImage::blank(256, 256);
    assert_eq!(
        paint_map_overlays(
            &source,
            &map,
...
fn empty_footprint_skips_when_same_image_anchor_neighbor_draws() {
    ...
    let source = MapSource { files };
    let mut map = MapInfo::empty(GameEdition::Ra2, "t");
    map.overlays = vec![OverlayCell { x: 5, y: 0, overlay_id: 1, data: 0 }, OverlayCell { x: 6, y: 0, overlay_id: 1, data: 1 }];
    let mut image = TerrainImage::blank(256, 256);
    let (shp, mark) = paint_map_overlays(
        &source,
        &map,
...
fn mixed_lobrdb_flank_does_not_paint_neighbor_deck() {
    ...
    let mut files = HashMap::new();
    files.insert("art.ini".into(), b"[LOBRDB12]\nTheater=yes\n[LOBRDB10]\nTheater=yes\n".to_vec());
    files.insert("isotem.pal".into(), solid_index_pal(5, 0, 63, 0));
    ...
    let mut image = TerrainImage::blank(256, 256);
    let (shp, mark) =
        paint_map_overlays(&source, &map, &mut image, &PaintDefinitions::load(&source, "art.ini", "rules.ini"), &name, &|_| false, &|_| None, OverlayLayerFilter::All);
...
fn bridge_layer_filter_skips_ore_on_bridge_pass() {
    ...
    let source = MapSource { files };
    let mut map = MapInfo::empty(GameEdition::Ra2, "t");
    map.overlays = vec![OverlayCell { x: 1, y: 0, overlay_id: 102, data: 0 }, OverlayCell { x: 2, y: 0, overlay_id: 1, data: 0 }];
    let name = |id: u8| match id {
        102 => Some("TIB01".into()),
        1 => Some("BRIDGE1".into()),
        _ => None,
    ...
    let (g_shp, g_mark) =
        paint_map_overlays(&source, &map, &mut ground, &PaintDefinitions::load(&source, "art.ini", "rules.ini"), &name, &|id| id == 102, &|_| None, OverlayLayerFilter::Ground);
    assert_eq!((g_shp, g_mark), (1, 0), "ground pass paints ore only");
    let mut bridge = TerrainImage::blank(256, 256);
    let (b_shp, b_mark) =
        paint_map_overlays(&source, &map, &mut bridge, &PaintDefinitions::load(&source, "art.ini", "rules.ini"), &name, &|id| id == 102, &|_| None, OverlayLayerFilter::Bridge);
...
fn paint_overlays_onto_preview_rgba_writes_selected_cells() {
    ...
    let paint = PaintDefinitions::load(&source, "art.ini", "rules.ini");
