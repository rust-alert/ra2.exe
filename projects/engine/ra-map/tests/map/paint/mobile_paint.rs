...
use ra_map::{MapInfo, MobilePaintPose, TerrainImage, paint_map_mobiles};
...
fn empty_mobiles_noop() {
    ...
    assert_eq!(
        paint_map_mobiles(&EmptySource, &map, &mut image, "art.ini", "rules.ini", &|p, _| p.clone(), &|_| MobilePaintPose::default(),),
        0
    );
}
