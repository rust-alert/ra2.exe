//! `[OverlayPack]` / `[OverlayDataPack]`：编号 pack → base64 → LCW 分块 → 512×512 格网。

use ra_assets::IniDocument;
use ra_types::{RaError, RaResult};

use crate::{lcw, numbered_pack::decode_numbered_base64_pack};
...
fn decode_pack_section(doc: &IniDocument, section: &str) -> RaResult<Vec<u8>> {
    let compressed = decode_numbered_base64_pack(doc, section)?;
