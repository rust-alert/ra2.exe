...
fn place_refinery_then_ore_trip_credits_through_session() {
    ...
    case.advance(1);
    let after_build = case.session.expect_game_mut().world.house_funds(slice.human_house).expect("应有资金");
    assert_eq!(after_build, slice.starting_funds - 600 - 2000);
    case.advance(u64::from(ORE_TRIP_TICKS));
    assert_eq!(case.session.expect_game_mut().world.house_funds(slice.human_house), Some(after_build + slice.ore_income_per_trip));
}
