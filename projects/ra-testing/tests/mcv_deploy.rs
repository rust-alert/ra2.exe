...
fn mcv_deploy_open_seeds_slice_funds() {
    ...
    assert_eq!(case.session.world.house_funds(slice.human_house), Some(slice.starting_funds));
    assert_eq!(case.session.world.entities[0].type_id, slice.allied_mcv);
}

#[test]
fn mcv_deploy_through_session_becomes_yard() {
    let mut case = mcv_deploy_open();
    let id = case.session.world.entities[0].id;
    case.command(GameCommand::Deploy { entity_index: 0 });
    case.advance(1);
    assert!(case.session.world.last_rejects().is_empty());
    assert_eq!(case.session.world.entities[0].id, id);
    assert_eq!(case.session.world.entities[0].kind, MapEntityKind::Structure);
    assert_eq!(case.session.world.entities[0].type_id, "GACNST");
