...
fn place_power_through_session_deducts_funds() {
    ...
    assert_eq!(case.session.world.entities.len(), 2);
    assert_eq!(case.session.world.entities[1].kind, MapEntityKind::Structure);
    assert_eq!(case.session.world.entities[1].type_id, "GAPOWR");
    assert_eq!(case.session.world.players[0].power_output, 200);
}
