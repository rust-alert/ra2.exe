...
fn nested_expand_leaf_overlays_base_leaf() {
    ...
    assert_eq!(src.mount_nested_plan(&[NestedMountSpec { name: "cache.mix".into(), strategy: NestedMountStrategy::AllParents }]), (2, 0));
...
fn discover_finds_stock_mp_name_in_mounted_mix() {
    ...
    let mut src = GameAssetSource::new(dir.clone());
    src.vfs.mount_bytes_with_meta("maps01.mix", mix, 0, None, Some("maps".into())).unwrap();
