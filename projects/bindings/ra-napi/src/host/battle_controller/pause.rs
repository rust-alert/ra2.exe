...
impl BattleController {
    ...
    pub(super) fn refresh_pause_hover(&mut self, window: &Window) {
        ...
        self.pause_hover =
            battle_pause_menu::hit_at(size.width.max(1), size.height.max(1), self.cursor.0 as i32, self.cursor.1 as i32).map(|h| h.entry_id());
    }
    ...
    pub(super) fn handle_pause_menu_mouse(&mut self, state: ElementState, window: &Window) -> BattleNav {
        ...
        match state {
            ElementState::Pressed => {
                self.pause_pressed = battle_pause_menu::hit_at(size.width.max(1), size.height.max(1), x, y).map(|h| h.entry_id());
                BattleNav::None
            }
            ElementState::Released => {
                let pressed = self.pause_pressed.take();
                let hit = battle_pause_menu::hit_at(size.width.max(1), size.height.max(1), x, y);
