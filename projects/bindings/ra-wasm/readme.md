...
当前导出 `version` / `engine_name` / `supports_present` / `attachCanvas` / `resizePresent` / `presentFrame` / `presentBackend`，以及 `InstallSession` / `PrepareReport` / `loadJobBusy` / `loadJobProgress`。画布经 wgpu 附着后 `supports_present` 为真。
