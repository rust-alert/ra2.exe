...
use ra_map::{Theater, parse_tileset_ini, theater_ini_name, theater_mix_names, theater_palette, theater_tmp_extension};
...
fn main() {
    ...
    let ini = vfs.read(theater_ini_name(Theater::Temperate)).expect("temperat.ini");
    ...
    let pal = Palette::parse(&vfs.read(theater_palette(Theater::Temperate)).expect("isotem.pal")).expect("pal");
    ...
    eprintln!("OK {name} cells={} tile={}x{} opaque_px={}", tmp.cell_count(), tmp.tile_width, tmp.tile_height, opaque);
}
