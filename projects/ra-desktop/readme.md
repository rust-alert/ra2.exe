...
工作区默认成员。本 crate 产出原生 GUI 二进制 **`rust-ra2`**（Windows 上为 **`rust-ra2.exe`**）。它是玩家与 **`ra-engine`**
...
进程读取 **可执行文件同目录**的 `RustAlert.toml`（由 `toml_edit` 解析），承载分辨率与其它启动选项。
默认情形：文件中未写 `ra2_dir`（或文件暂缺）时，`ra2_dir` 为 exe 所在目录、`edition = None`，便于把 `rust-ra2.exe` 放进游戏安装目录定位资源；其它选项仍依赖本配置文件。
