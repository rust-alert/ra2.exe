//! 原生 GUI 入口：二进制名 `rust-ra2`（Windows 上为 `rust-ra2.exe`）。
//!
//! 不是命令行工具——启动配置来自可执行文件同目录的 `RustAlert.toml`，然后由窗口接管进程。
...
fn init_tracing() -> WorkerGuard {
    ...
    let file_appender = tracing_appender::rolling::never("logs", "rust-ra2.log");
