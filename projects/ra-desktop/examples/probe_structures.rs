...
use ra_map::{MapEntityKind, MapInfo, new_theater_shp_name, theater_mix_names};
...
fn probe(root: &Path, edition: GameEdition) -> RaResult<()> {
    ...
    let art = IniDocument::parse(&vfs.read("art.ini").ok_or_else(|| RaError::MissingFile("art.ini".into()))?)?;
    ...
    for type_id in &types {
        ...
        let new_theater = art.get(type_id, "NewTheater").is_some_and(|v| v.eq_ignore_ascii_case("yes"));
        let candidates = if new_theater {
            vec![new_theater_shp_name(image, map.theater), format!("{}.shp", image.to_ascii_lowercase())]
        }
        else {
            vec![format!("{}.shp", image.to_ascii_lowercase()), new_theater_shp_name(image, map.theater)]
        };
    ...
    eprintln!("OK structures_placed={} types={} shp_types={} theater={}", placed, types.len(), resolved, map.theater.as_str());
    Ok(())
}
