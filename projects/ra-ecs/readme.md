...
1. 弄清 ECS 在本项目中的边界与 non-goals（「它是什么」）。
2. 理解 `ra-engine` 如何消费本 crate（「在仓库中的位置」）。
3. 在引擎模块中引用 `EcsEntity` / `EcsWorld`（「如何使用」）。
4. 了解槽位—世代模型与命令缓冲的设计意图（「内部设计」）。
5. 构建、测试与许可证（文末）。
...
- **与 `ra-definition` 正交**：定义契约描述规则表形状；ECS 描述运行时实体存储机制。
