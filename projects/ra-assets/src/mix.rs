...
use crate::mix_crypto::{self, BLOWFISH_BLOCK_SIZE, RSA_KEY_BLOCK_SIZE};
use crate::mix_hash::mix_hash;
...
fn parse_encrypted_new_format(data: &[u8]) -> RaResult<(Vec<MixEntry>, usize)> {
    ...
    let encrypted_size =
        total_index_bytes.div_ceil(BLOWFISH_BLOCK_SIZE) * BLOWFISH_BLOCK_SIZE;
