...
pub fn extract_blowfish_key(encrypted_block: &[u8]) -> RaResult<[u8; BLOWFISH_KEY_SIZE]> {
...
pub fn blowfish_decrypt_ecb(key: &[u8], data: &mut [u8]) -> RaResult<()> {
    if !data.len().is_multiple_of(BLOWFISH_BLOCK_SIZE) {
        return Err(RaError::Parse(format!(
            "Blowfish 数据长度 {} 不是 {} 的倍数",
            data.len(),
            BLOWFISH_BLOCK_SIZE
        )));
    }

    let cipher: Blowfish<BE> = Blowfish::new_from_slice(key)
        .map_err(|e| RaError::Parse(format!("Blowfish 初始化失败: {e}")))?;

    for chunk in data.chunks_exact_mut(BLOWFISH_BLOCK_SIZE) {
        let block = blowfish::cipher::Block::<Blowfish<BE>>::from_mut_slice(chunk);
...
mod tests {
    ...
    fn blowfish_roundtrip_aligned() {
        ...
        {
            let block = blowfish::cipher::Block::<Blowfish<BE>>::from_mut_slice(&mut data);
