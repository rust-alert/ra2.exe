...
`GameCommand` 现含 `MoveTo`、`Attack`、`Deploy` 与 `PlaceBuilding`。实体已带稳定 `EntityId`，命令暂仍可用下标或 `PlayerId` 寻址。`PlayerState`、资金扣除、建造场/供电前置与 `CommandReject` 已就位。存活矿场按 `ORE_TRIP_TICKS` 周期入账 `ORE_INCOME_PER_TRIP`。生产队列尚未实现。
